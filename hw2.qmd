...
df_quant_histogram <- df %>%
  pivot_longer(!sex, names_to = "stats", values_to = "count")
...
There is a notable difference between infant abalones as opposed to male
and female abalones with infant abalones having a lower length and
diameter on average(which is expected). However, male and female
abalones have no notable differences in their physical appearance.
...
There are a few notable anomalies in the dataset. One is an infant
abalone which has an uncharacteristically high diameter and shell weight
for its length. There is also a male abalone with an
uncharacteristically low diameter for its length.
...
high_length_abalone <- df %>% 
  group_by(sex) %>% 
  filter(length >= 0.5) %>%
  summarise(across(c(length, diameter, height, whole_weight, shucked_weight, viscera_weight, shell_weight, rings), mean, na.rm=TRUE))

high_length_longer <- high_length_abalone %>% pivot_longer(!sex, names_to = "stats", values_to = "values")

ggplot(data = high_length_longer, aes(x=stats, y=values)) +
  geom_col()+
  facet_wrap(~sex,ncol=1)
...
ring_count <- df %>%
  ...
  summarise(avg_weight=mean(whole_weight)+mean(shucked_weight)+mean(viscera_weight)+mean(shell_weight))

ring_count %>%
  ggplot(aes(x=sex, y=num_rings)) +
...
df_num <- df %>% select(length, diameter, height, whole_weight, shucked_weight, viscera_weight, shell_weight, rings)
...
df %>% 
  select(c(length, diameter, height, whole_weight, shucked_weight, viscera_weight, shell_weight, rings)) %>%
  map2(., colnames(.), ~{
    ggplot(df) +
...
lm_diameter <- lm(height ~ diameter, data = df)

summary(lm_diameter)


```
The intercept being slightly negative at -0.003803 indicates that the regression line hits the y-axis at a point where y < 0.  Additionally, the diameter coefficient being at 0.351376 estimates a increase of that much per unit of height.  The p-value being much less than 0.05 indicates significance in this relationship.
...
```
The linear model is generally an appropriate fit for this relationship becauseit represents most of the points where are either along the line or around it.  The points also show a positive linear relationship just like the line predicts.  
...
new_y <- predict(project,new_data= new_diameters)

final_x <- rbind(x, new_diameters)
final_y <- rbind(y, new_y)

points(final_x, final_y)
abline(lm(final_y ~ final_x), col='violet')
